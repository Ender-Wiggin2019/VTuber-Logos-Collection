...
const CarouselContent = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => {
  ...
  return (
    <div ref={carouselRef} className="overflow-hidden">
      <div ref={ref} className={cn("flex", orientation === "horizontal" ? "-ml-4" : "-mt-4 flex flex-col", className)} {...props} />
    </div>
  );
});
...
const CarouselPrevious = React.forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(({ className, variant = "outline", size = "icon", ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant={variant}
      size={size}
      className={cn("absolute  h-8 w-8 rounded-full", orientation === "horizontal" ? "-left-8 top-1/2 -translate-y-1/2" : "-top-8 left-1/2 -translate-x-1/2 rotate-90", className)}
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        orientation === "horizontal" ? "-right-8 top-1/2 -translate-y-1/2" : "-bottom-8 left-1/2 -translate-x-1/2 rotate-90",
        className
      )}
